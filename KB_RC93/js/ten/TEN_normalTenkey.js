/*************************************************************************************
	...
	Revision History	:	
         ...
         ver 1.0.0.4 - 재배열 추가(:4 , 키패드1~9 랜덤배열)
**************************************************************************************/
...
//function evtStart(){
    ...
    tenClass.defaultGab = tenObj.paramArr[11]!=undefined?tenObj.paramArr[11].split(":")[1]:tenObj.defaultGab;//초기값 세팅이 필요할경우
    tenClass.firstNational = tenObj.paramArr[12]!=undefined?tenObj.paramArr[12].split(":")[1]:tenObj.firstNational;//강제로 텐키관련 언어지정을 위한 변수
    ...
    tenClass.evt()
//}
function TEN_normalTenkey() {//생성자함수
    ...
    let aster = "", asterNum = 0, asterNum2 = 0, astermore = 0,asterPosition = '',jumin = 0, dxtIndex2 = 0, confirmNumber = 0, linkNum = 0, paramG="";
    ...
    this.popupFlag = 0;
    this.connectF = 0;
    this.defaultGab = "";///처음 텐키 텍스트창에 세팅할값을 담는 변수
    this.firstNational = ""//강제로 텐키관련 언어지정을 위한 변수
    let txtNum = 0;
    let triggerF = false;
    ...
    this.evt = function(){
        ...
        pageNumber = pageNum.substr(2,pageNum.length);///현재페이지명중 숫자만 담는 변수
        //console.log('pageNum===='+pageNum,'pageNumber ='+pageNumber)//
        //console.log('this.lge===='+this.lge)
        if(this.firstNational !=undefined && this.firstNational != ""){//강제로 텐키관련 언어지정을 위한 변수
            national = this.firstNational.trim();
        }else{
        ...
        //national = 'ru'////테스트용
        if(this.defaultGab !=undefined){//확인키 위치값 재설정을 위한 분기문
            if(this.defaultGab.indexOf('/')>-1 && this.defaultGab.indexOf('-')>-1){//확인키 위치값 재설정을 위한 분기문
                confirmLocation.top = this.defaultGab.split('/')[1].split('-')[0];
                confirmLocation.left = this.defaultGab.split('/')[1].split('-')[1];
            }
        }
        let nationalWord ={
        ...
        if(String(this.maxGab).indexOf('-')>-1){
            ...
            if(this.maxGab.indexOf('*')>-1){/////숫자와 아스타 혼용을 위한부분
                astermore = this.maxGab.indexOf('*') != this.maxGab.lastIndexOf('*') || this.maxGab.indexOf('N') != this.maxGab.lastIndexOf('N') ?2:1;///*가 1개면 1, *가 2개면 2
                if(astermore==1){
                    asterPosition = this.maxGab.length-1 == this.maxGab.indexOf('*')?'back':'front';///back이면 *가뒤쪽에, front이면 *가 앞쪽에
                }else if(astermore==2){
                    asterPosition = this.maxGab.length-1 == this.maxGab.lastIndexOf('*')?'front':'back';
                }
                if(asterPosition == 'front'){
                    ...
                    if(astermore == 1){
                        ...
                        asterNum2 = Number(this.maxGab.split('*')[1].split('N')[0])
                        this.maxGab = asterNum+asterNum2+Number(this.maxGab.split('*')[1].split('N')[1])
                    }
                }else if(asterPosition == 'back'){
                    asterNum = Number(this.maxGab.substring(this.maxGab.indexOf('N')+1,this.maxGab.indexOf('*')));
                    if(astermore == 1){
                        this.maxGab=Number(this.maxGab.split('N')[0])+asterNum
                    }else if(astermore == 2){
                        asterNum2 = Number(this.maxGab.substring(this.maxGab.indexOf('*')+1,this.maxGab.lastIndexOf('N')))
                        this.maxGab = Number(this.maxGab.split('N')[0])+asterNum+asterNum2;//3N4*9N
                    }
                }
            }
            this.dashGab = "";
        ...
        if(this.allDimdF==1){
            ...
            if(spArrNumber==-1){////다른js와 연동이 아닌경우
                tenClass.cal('clear',tenClass.dxtIndex);
                if(this.defaultGab !=undefined){
                    if(this.defaultGab !="" && this.defaultGab.indexOf('/')<0){///초기값이 있을경우(참고로 확인키 위치값 고정을 없애기위해 값과 같은 속성을 쓰기때문에 '-'인는지 확인함
                        tenClass.cal(this.defaultGab,tenClass.dxtIndex);
                     }else if(this.defaultGab !="" && this.defaultGab.indexOf('/')>-1 && this.defaultGab.split('/')[0] != ''){
                        tenClass.cal(this.defaultGab.split('/')[0],tenClass.dxtIndex);
                     }
                }
                this.confirmDimd(1); 
            }else{///다른js와연동인경우
                paramG == ""?tenClass.cal('clear',tenClass.dxtIndex):"";//다른js와 연동이지만 특정페이지(1106)이 아닌경우에만 실행 paramG는 특정페이지(1106)에 값이 바뀌는 변수임
            }
        }
        let shuffleF = tenClass.shuffle===1?this.keyDraw():tenClass.shuffle===4?this.keyRandom():"";
    ...
    ////재배열4일시 버튼위치세팅 함수
    this.keyRandom=function(){
        let randomArr = [];
        let randomCss = [];
  
        for(var i=0; i<9; i++){
            var cssArr = [];
            cssArr.push($('#ten'+i).css('left'));
            cssArr.push($('#ten'+i).css('top'));
            randomCss.push(cssArr);
        };
        console.log('randomCss =='+randomCss)
        for(i=0; i<9; i++){
            var ranArr1 = Math.floor(Math.random()*(9)); //0~8까지
            if(randomArr.indexOf(ranArr1) <0){
                randomArr.push(ranArr1);
                var cssJsonr = {
                    'left': randomCss[randomArr[i]][0],
                    'top': randomCss[randomArr[i]][1]
                }
                $('#ten'+i).css(cssJsonr);
            }else{
                i--;
    ...
    this.commaAndNormal = function(dxtIndex){
        ...
        if(this.comma){
            ...
            let changeGab = ""
            if(asterNum != 0 && astermore == 1){//아스타혹은 N표시가 하나인경우
                ...
                if(this.tmpGab=="")$("#dxt" + dxtIndex).text(this.tmpGab);
            }else if(asterNum != 0 && astermore == 2){//아스타혹은 N표시가 두개인경우
                let firstNum  = this.maxGab - asterNum - asterNum2
                let lastNum = asterNum+asterNum2
                for(var i = 0 ; i<this.tmpGab.length ; i++){
                    if(asterPosition == 'front'){
                        if(i<asterNum){
                            changeGab = changeGab+"∗"//this.tmpGab.substr(i,1)
                            $("#dxt" + dxtIndex).text(changeGab);
                        }else if(asterNum<=i && i<lastNum){
                            $("#dxt" + dxtIndex).text(changeGab+this.tmpGab.slice(asterNum,this.tmpGab.length));
                            if(i==lastNum-1){
                                changeGab = changeGab+this.tmpGab.slice(asterNum,lastNum)
                            }
                        }else if(lastNum<=i){
                            changeGab = changeGab+"∗"
                            $("#dxt" + dxtIndex).text(changeGab);
                        ...
                        if(i<firstNum){
                            $("#dxt" + dxtIndex).text(this.tmpGab);
                        }else if(firstNum <=i && i<this.maxGab - asterNum2){
                            changeGab = changeGab+"∗"
                            $("#dxt" + dxtIndex).text(this.tmpGab.slice(0,firstNum)+changeGab);
                            if(i==firstNum+asterNum-1){
                                changeGab = this.tmpGab.slice(0,firstNum)+changeGab;
                            }
                        }else{
                            $("#dxt" + dxtIndex).text(changeGab+this.tmpGab.slice(firstNum+asterNum,this.maxGab.length));
                        } 
                    }
                }
                if(this.tmpGab=="")$("#dxt" + dxtIndex).text(this.tmpGab);
    ...
    this.cal = function (gab, dxtIndex,outNum) {/////////outNum매개변수는 다른js에서 cal함수를 호출유무에 관한것으로 없는것은 텐키에서 호출, 있는것은 다른js파일에서 호출 
        let asterAndNum = ""//아스타, 숫자 혼합할때 쓰이는 변수
        let outFlag = outNum, pageFlag = spName+pageNum.substr(2,pageNum.length);
        ...
        switch (gab) {
            ...
            case "back": //지우기 버튼으로 들어오면
                ...
                if(this.dashGab == ""){
                    ...
                    }else if (this.pwFlag=="3"){
                        asterAndNum = aster.substr(0,aster.length-1)+this.tmpGab.substr(this.tmpGab.length-1,1)
                        $("#dxt" + dxtIndex).text(asterAndNum);
                    }else{
            ...
            default:
                ...
                if((this.manchon && tenObj.lge == 'kr') || (this.manchon && tenObj.lge == 'lv')){
                    ...
                    this.manchon?this.unitButton_Disabled():'';
                }
                break;
        }
    ...
    this.returnGab = function(){//////////////////////////////////////////////////////////////////////////////////다른 JS하고 연결하고 쓸 함수임
        //let pageFlag = spName+''+pageNum.substr(2,pageNum.length)
        let pageFlag = spName+''+pageNum.replace(/[^0-9]/g, "");
        if(spFlag){//////////////connectF가 true시 즉 tenCall함수 사용해야 하는 상황
            switch(pageFlag){
                case spName+'10102'://추후추가 할것리 있으면 추가 default와 같음.
                     console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
                break;
                case spName+'10103'://추후추가 할것리 있으면 추가 default와 같음.
                    console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
                break;
                case spName+'10310':
                ...
                case spName+'10801':
                    console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
                break;
                case spName+'20003':// SMC
                     console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
                break;
                case spName+'20020':// SMC
                     console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
                break;
                case spName+'20025':// SMC
                     console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
                break;
                default:
            ...
            switch(pageFlag){
                case spName+'10102':   
                     console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + sumReturn + "]";
                break;
                case spName+'10103':
                    console.log("tenObj.outReturnG==="+tenObj.outReturnG)
                     btnArr[confirmNumber].breturn = "=[" + tenObj.outReturnG + "]";
    ...
    this.unitButton_Disabled = function(){	
        ...
        ////다국어용 10000,1000버튼 관련
        if(my_txt == ""){
            this.tenE("10000",true)
            this.tenE("1000",true)
        }else{
            this.tenE("10000",false)
            this.tenE("1000",false)
        }
