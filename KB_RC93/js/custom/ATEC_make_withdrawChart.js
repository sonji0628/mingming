/*************************************************************************************
	FileName			:	ATEC_make_withdrawChart.js
	Description			:	메인화면
                            출금부 정보                           
...
for (var i = 0; i < cmArr.length; i++) {
	if (cmArr[i].class == "make_withdrawChart") { //요기 바꿔주고
		cmArr[i].jsObj = new ATEC_make_withdrawChart( Number(cmArr[i].index) ); //요기 바꿔주고
		cmArr[i].jsObj.evt();
	}
}


function ATEC_make_withdrawChart(ind) {
    ...
    //evtParam DATA: 총출금가능금액;현금출금가능금액;동전출금가능금액;권종1|최대1|잔량1;………권종8최대8|잔량8;
    //sendMsg`WithdInfo`1000000000;99000000;2000000;50000|3800|1000;10000|3800|2000;5000|1500|300;1000|1500|200;500|1500|0;100|2500|200;50|3000|50;10|3500|100;
	...
	this.evt = function() {
                                ...
                                <div class='myMoneyState'>
                                    <ul>
                                        <li class='myMoneyState_option'>
                                            <span class='title'>현금 출금 가능금액</span>
                                            <span class='money'></span>
                                        </li>
                                        <li class='myMoneyState_option'>
                                            <span class='title'>동전 출금 가능금액</span>
                                            <span class='money'></span>
                                        </li>
                                        <li class='myMoneyState_option'>
                                            <span class='title'>총 출금 가능금액</span>
                                            <span class='money'></span>
                                        </li>
                                    </ul>
                                </div>
        ...
        myStateInfoArea =  document.querySelector("#withdrawArea > div.info > div.myMoneyState > ul");//금액 현황 영역
        myChartArea =  document.querySelector("#withdrawArea > div.myChart > ul");//차트영역
    ...
    this.callF = function(gab) { //후처리 함수
        ...
        let withdrawTotalCashArea = myStateInfoArea.querySelectorAll(".myMoneyState_option > .money"); //출금가능 금액 현황 입력할 영역
        let myRemain = myChartArea.querySelectorAll("li > div.myStateText > p.myData");         //잔량 영역
        let withdrawTotalCash = Number(arrList[0]).toLocaleString(); //총 출금 가능 금액 데이터
        let cashTotal = Number(arrList[1]).toLocaleString();         //현금 출금 가능 금액 데이터
        let coinTotal = Number(arrList[2]).toLocaleString();         //동전 출금 가능 금액 데이터
        ...
        if("" != gab) {
            withdrawTotalCashArea[0].innerText = cashTotal;         //★ 출금가능금액 현황 입력
            withdrawTotalCashArea[1].innerText = coinTotal;         //★ 출금가능금액 현황 입력
            withdrawTotalCashArea[2].innerText = withdrawTotalCash; //★ 출금가능금액 현황 입력
            ...
            function dataSet(i){
                ...
                myGraphheight = Math.round(myMoneyDataArr[2] / myMoneyDataArr[1] * 100)*0.8;//권종별 그래프 세로값 백분율 정수 계산
