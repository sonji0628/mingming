/*************************************************************************************
    FileName			:	ATEC_makeTableCash.js
    Description			:	테이블 생성
...
for (var i = 0; i < cmArr.length; i++) {
	if(cmArr[i].class == "makeTableCash") { //요기 바꿔주고
		cmArr[i].jsObj = new ATEC_makeTableCash(Number(cmArr[i].index)); //요기 바꿔주고
		cmArr[i].jsObj.evt();
	}
}

function ATEC_makeTableCash(ind) {
    ...
    let myTable;   //권종별 테이블
    let label = ['오 만 원','만 원','오 천 원','천 원'];
    let labelNum = [50000,10000,5000,1000];
    let myColor = ['#d8ae66','#d8ae66','#d8ae66','#d8ae66'];//권종별 그래프 색상
