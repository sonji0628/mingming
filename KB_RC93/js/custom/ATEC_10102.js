...
for (var i = 0; i < cmArr.length; i++) {
	if(cmArr[i].class == "10102") { //요기 바꿔주고
		cmArr[i].jsObj = new ATEC_10102(Number(cmArr[i].index)); //요기 바꿔주고
		cmArr[i].jsObj.evt();
...
function ATEC_10102(ind) {
    ...
    let notice = `<div class='pop_notice' style='display:none'><span>출금할 수 있는 최대 매수를 초과하였습니다.</span></div>`
