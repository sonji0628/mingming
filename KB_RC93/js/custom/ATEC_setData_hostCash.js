/*************************************************************************************
	FileName			:	ATEC_setData_hostCashSet.js
	Description			:	메인화면
                            단말시재 데이터 셋팅                           
...
for (var i = 0; i < cmArr.length; i++) {
	if(cmArr[i].class == "setData_hostCash") { //요기 바꿔주고
		cmArr[i].jsObj = new ATEC_setData_hostCash(Number(cmArr[i].index)); //요기 바꿔주고
		cmArr[i].jsObj.evt();
	}
}

function ATEC_setData_hostCash(ind) {
    this.iname = $(cmArr[ind].xmlObj).attr("iname");

    //evtParam DATA: 총금액;권종1|금액1;권종2|금액2;……권종8|금액8;
	...
	this.callF = function (gab) {
        ...
        // hostTotalCash.innerText = totalCash; //★ 총금액 시재 현황 입력
        // for(let i = 0; i < cashUnits.length; i++){
        //     cashUnitData = cashUnits[i].split("|");    //권종별 데이터를 '|' 구분자로 배열화
        //     hostCashs[i].innerText = Number(cashUnitData[1]).toLocaleString(); //★ 권종별 시재 현황 입력
        // }
	}
}