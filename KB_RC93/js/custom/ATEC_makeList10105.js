/*************************************************************************************
    FileName			:	ATEC_makeList10105.js
    Description			:	세로 리스트 생성
...
for (var i = 0; i < cmArr.length; i++) {
	if(cmArr[i].class == "makeList10105") { //요기 바꿔주고
		cmArr[i].jsObj = new ATEC_makeList10105(Number(cmArr[i].index)); //요기 바꿔주고
		cmArr[i].jsObj.evt();
	}
}

function ATEC_makeList10105(ind) {
    ...
    this.callF = function(gab) { //후처리 함수
        ...
        if("" != gab) {
            ...
            for(let i = 0; i < arrList.length; i++){
                ...
                $(cashList[i]).unbind();
                $(cashList[i]).bind('mousedown',listSelectBtn); //클릭이벤트
            }
    ...
    function create_btnNextPrev(){
        ...
        myPageNum = $(`<div class='pageNum'></div>`)
        
        $("#cm"+ind).append(nextPrevBtn); 
        $(nextPrevBtn).append(btnPrev);
        $(nextPrevBtn).append(myPageNum);
        ...
        btnPrev.addEventListener("mousedown",pagingBtn);
        btnNext.addEventListener("mousedown",pagingBtn);
    ...
    function showPage() {
        ...
        myPageNum[0].innerText = page + " of " + (pagenum + 1);
    ...
    function listSelectBtn() {
        ...
        if (!eventFlag && mybtn.attr('dimd') != '1'){ //다른게 눌리지 않아야지 됨, 딤드된 버튼은 눌리지 않아야지 됨
            ...
            //
            $(".cashList[dimd='0']").css({'background':'#fff','border': '1px solid #bbbbbb','box-shadow':'none'});
            mybtn.css({'background':'#fffde5','box-shadow': '0 0 0 4px #ffbc00','border': '1px solid #ffbc00'});
