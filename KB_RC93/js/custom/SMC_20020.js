/*************************************************************************************
    FileName			:	ATEC_20020.js
    Description			:	smc 시재장착
...
for (var i = 0; i < cmArr.length; i++) {
	if(cmArr[i].class == "20020") { //요기 바꿔주고
		cmArr[i].jsObj = new SMC_20020(Number(cmArr[i].index)); //요기 바꿔주고
		cmArr[i].jsObj.evt();
	}
}

function SMC_20020(ind) {
    ...
    //테이블 그려주기
    mySection = $(`<div id='mySection' class='smc20020'>
                    <div class='cashTable'>
                        <ul>
                            <li class='table_title'>
                                <div class='title'></div>
                                <div class='nowAmount'>현재수량</div>
                                <div class='nowMoney'>현재금액</div>
                                <div class='newAmount'>보충수량</div>
                                <div class='newMoney'>보충금액</div>
                            </li>
                    ...
                    <div class='totalWrap'>
                        <div class='wrap'>
                            <div class='nowTotal'>보충 전 <span> </span></div>
                            <div class='newTotal'>보충 중 <span> </span></div>
                            <div class='totalSum'>보충 후 <span> </span></div>
                        </div>
                    </div>
                </div>`);
    ...
    function create_cashTable_table(i){
        ...
        myTable = $(`<li class='cashUnit' dimd='0'>
                    ...
                    <div class='nowMoney'><span> </span></div>
                    <div class='newAmount'><span> </span></div>
                    <div class='newMoney'><span> </span></div>
                </li>`);
    ...
    let nowMoney = document.querySelectorAll(".nowMoney > span");        //해당 권종의 현재 금액 div의 입력칸
    let newAmount = document.querySelectorAll(".newAmount > span");      //해당 권종의 보충 매수 div의 입력칸
    let newMoney = document.querySelectorAll(".newMoney > span");        //해당 권종의 보충 금액 div의 입력칸
    let cashWrap = document.querySelector(".cashWrap");
    let coinWrap = document.querySelector(".coinWrap");

    let nowTotal = document.querySelector(".nowTotal > span");
    let newTotal = document.querySelector(".newTotal > span");
    let totalSum = document.querySelector(".totalSum > span");
    ...
    this.callF = function(gab) { //후처리 함수
        ...
        if("" != gab) {
            ...
            for(let i = 0; i < arrList.length; i++){
                ...
                nowTotalSum += myMoneyData;
            }
            nowTotal.innerText = nowTotalSum.toLocaleString();
            newTotal.innerText = Number(0);
            totalSum.innerText = nowTotalSum.toLocaleString();
    ...
    function dataSet(i){
        ...
        nowMoney[i].innerText = (cashUnitData[1]*cashUnitData[0]).toLocaleString();
        newAmount[i].innerText = Number(0);  // 권종별 금액 현황 입력
        newMoney[i].innerText = Number(0);  // 권종별 금액 현황 입력

        myMoneyData = cashUnitData[1]*cashUnitData[0];
    ...
    function inputDataSet(input){   
        ...
        if(selectFlag){
            ...
            newMoney[selectIndexNum].innerText =  (Number(tenClass.tmpGab) * selectMoney).toLocaleString(); 

            //보충 총 금액 셋팅
            let newTotalSum = 0;
            newMoney.forEach(element => newTotalSum += Number(element.innerText.replace(/,/g, "")));
            newTotal.innerText = newTotalSum.toLocaleString();
            //보충 전, 후 금액 합 셋팅
            totalSum.innerText = (nowTotalSum + newTotalSum).toLocaleString();
        }
