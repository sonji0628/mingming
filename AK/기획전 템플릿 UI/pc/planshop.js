...
function planshopView(){
    ...
    function fadeRollImg(el){
        ...
        case "fadeType" :
            ...
            let floatWrap = el.querySelectorAll(".float_wrap");
            let i = 0;
            let count = 0;
            fadeTimeout();
            function fadeTimeout(){
                imgItem.forEach(ele => {
                    ele.setAttribute("data-hidden", "true");
                    ele.classList.remove("move");
                });
                floatWrap.forEach(ele => {
                    ele.children[i].setAttribute("data-hidden", "false");
                    ele.children[i].classList.add("move");
                });
                i = i === (floatWrap[0].children.length - 1) ? 0 : i + 1;
            
                let myTimeout = count !== 0 ? 3000 : 100;
                setTimeout(fadeTimeout, myTimeout);
                count = 1;
            }
        break;
        case "slideType" :
